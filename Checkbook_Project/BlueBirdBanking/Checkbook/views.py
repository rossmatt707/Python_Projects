...
def balance(request, pk):
    ...
    transactions = Transaction.objects.filter(account=pk)  # Retrieve all of that account's transactions
...
def transaction(request):
    ...
    if request.method == 'POST':
        if form.is_valid():  # Check to see if the submitted form is valid and if so, saves the form
            pk = request.POST['account'] # Retrieve which account hte transaction was for
